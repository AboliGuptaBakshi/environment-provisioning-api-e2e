...
1. Client submits an environment request.
2. API validates it and accepts valid work, returning an environment ID and operation ID.
3. Provisioning progresses asynchronously to a terminal success or failure state.
4. Client polls the operation and reads the environment and its resources.

Exact paths and payload fields will be fixed when implementing the API, before writing the corresponding tests.
...
| R4 | Invalid or unsupported input is rejected with useful validation details and creates no environment or operation. | Submit representative invalid requests; assert field errors and query that no state was created. |
...
The CI workflow will install declared Python dependencies and run the end-to-end suite. A nonzero test result fails the workflow; merge/readiness gate is a passing workflow. The exact workflow configuration will be added with the implementation.
